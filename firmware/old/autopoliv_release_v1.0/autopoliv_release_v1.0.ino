...
//--------------------------------------------------------------------------------------//
unsigned long k, t_for_j, t_for_d, t_for_sleep; // несколько вспомогательных переменных
int m, flagM, flag_forM;// несколько вспомогательных переменных
bool flag{1}, flagYorN, w; // несколько вспомогательных переменных
String last_input[6]{"", "", "", "", "", ""}, combin[6]{"up", "up", "down", "down", "down", "down"};
//--------------------------------------------------------------------------------------//
bool pause[4], checkP[4], pw[4]{1,1,1,1}, wtit[4]{0,0,0,0}; // пауза
String nm[4]{"Plant 1","Plant 2","Plant 3","Plant 4"}; // название растений
unsigned long con[4]; 
//--------------------------------------------------------------------------------------//
unsigned long t[4]{0, 0, 0, 0}; // таймер для каждого реле
unsigned long d[4]{0, 0, 0, 0}; // задержка между поливом
unsigned long td[4]{0, 0, 0, 0}; // время полива
double l[4]{0, 0, 0, 0}; // кол литров для каждого растения
int x = analogRead(A3);
int y = analogRead(A2);
double liter_per_minute{1.2}; //производительность насосов
//--------------------------------------------------------------------------------------//
GButton butt(6);
//--------------------------------------------------------------------------------------//
void setup() {
...
void preparing(){
  for(int i;i<4;i++){
    d[i]*=3600000;
    td[i] = l[i]/liter_per_minute*3600000/60;
    t[i] = millis();
    wtit[i] = !wtit[i];
  }
  t_for_sleep = millis();
...
void pomptick(){ // проходим по каждому растению
  for(int i{0};i < 4;i++){
    ...
    if(millis()-t[i]>=d[i]*wtit[i] && pause[i] == 0){
        powerDigit(i,0);
      if(millis()-t[i]>=d[i]*wtit[i]+td[i]){
        powerDigit(i,1);
        t[i] = millis();
        wtit[i] = 1;
      }
...
bool right(){ // проверка положения джойстика
  int x = analogRead(A2);
  int y = analogRead(A3);
  if(y>=800 && x<=700 && x>=350 && millis()-t_for_j>=250){
...
bool down(){ // проверка положения джойстика
  ...
  int y = analogRead(A3);
  //Serial.println(x,y);
  if(x<=100 && y<=700 && y>=350 && millis()-t_for_j>=250){
    Serial.println("down!");
    t_for_j = millis();
    gamest("down");
    return(true);
...
bool up(){ // проверка положения джойстика
  ...
  int y = analogRead(A3);
  //Serial.println(x,y);
  if(x>=800 && y<=700 && y>=350 && millis()-t_for_j>=250){
    Serial.println("up!");
    t_for_j = millis();
    gamest("up");
    return(true);
...
bool left(){ // проверка положения джойстика
  ...
  int y = analogRead(A3);
  //Serial.println(x,y);
  if(y<=100 && x<=700 && x>=350 && millis()-t_for_j>=250){
    Serial.println("left!");
    t_for_j = millis();
    gamest("left");
    return(true);
...
void d_h_m(int i, int m){
  ...
  else if(millis()-t[i]>=d[i]*wtit[i]) lcd.print("watering");
...
void sleep(){ //энергосбережение
  ...
  if(millis()-t_for_sleep>=50000){
    lcd.noBacklight();
  }
}
void gamest(String mvment){ //запуск игры
  for(int i{0}; i < 5; i++){ 
    String bufer = last_input[i];
    last_input[i] = last_input[i+1];
    last_input[i+1] = bufer;
  }
  last_input[5] = mvment;
  if(last_input == combin) Serial.print("Starting Game..."); //game();
}